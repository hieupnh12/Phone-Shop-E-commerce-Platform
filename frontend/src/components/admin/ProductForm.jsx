...
const ProductForm = ({
  ...
  useEffect(() => {
    if (!product && !formData.idProduct) {
      ...
      if (product.productVersionResponses && product.productVersionResponses.length > 0) {
        setVersions(product.productVersionResponses.map(v => ({
          idRam: v.ramName ? ramList.find(r => r.nameRam === v.ramName)?.idRam : '',
          idRom: v.romName ? romList.find(r => r.nameRom === v.romName)?.idRom : '',
          idColor: v.colorName ? colorList.find(c => c.nameColor === v.colorName)?.idColor : '',
  ...
  }, [product, brandList, originList, operatingSystemList, warehouseList, categoryList, ramList, romList, colorList]);
  ...
  const initializeProduct = async () => {
    try {
      ...
      setToast({ type: 'error', message: 'Lỗi khởi tạo sản phẩm: ' + error.message });
    } finally {
      setIsInitializing(false);
    }
  };
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setToast({ type: 'error', message: 'Kích thước hình ảnh không vượt quá 5MB' });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const productPayload = {
      ...
      products: {
        ...
        warrantyPeriod: formData.warrantyPeriod ? parseInt(formData.warrantyPeriod) : 0,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              {originList.map(origin => (
                <option key={origin.idOrigin} value={origin.idOrigin}>
                  {origin.nameOrigin}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            >
              ...
              {operatingSystemList.map(os => (
                <option key={os.idOperatingSystem} value={os.idOperatingSystem}>
                  {os.nameOperatingSystem}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <select
              ...
              value={formData.warehouseAreaId}
              onChange={handleInputChange}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              required
            >
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center gap-3">
          ...
          />
          <label htmlFor="productStatus" className="text-sm font-medium text-gray-700 cursor-pointer">
